...
def run_coordinator(nodes, steps, pop, rank, sigma, lr, seed):
    ...
    for proc, sink in zip(workers, stderr_sinks, strict=True):
    ...
    print("Multi-process cluster ZeroGrad")
    ...
    for i in range(1, len(worker_params)):
        for a, b in zip(worker_params[0], worker_params[i], strict=True):
    ...
    baseline_match = True
    for a, b in zip(worker_params[0], single_leaves, strict=True):
        diff = np.max(np.abs(a - b))
        if diff > 1e-5:
    ...
    print("  Params communicated: 0 bytes")
    print("  Each worker computed params locally from seed + loss history")
