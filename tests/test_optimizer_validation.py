...
class TestConstructorValidation:
    def test_rejects_non_transform_first_arg(self):
        with pytest.raises(TypeError):
            ZeroGrad("not-a-transform", 4, 2, 0.1, 42, "test")  # type: ignore[arg-type]
...
class TestInitAndStepValidation:
    ...
    def test_step_rejects_non_state(self):
        opt = _make_opt()
        with pytest.raises(TypeError):
            opt.step("not-a-state", _model(), None, _trivial_loss_fn)  # type: ignore[arg-type]

    def test_step_rejects_non_module(self):
        opt = _make_opt(manifest=_manifest())
        state = opt.init(_params())
        with pytest.raises(TypeError):
            opt.step(state, _params(), None, _trivial_loss_fn)  # type: ignore[arg-type]
...
class TestEvaluateShard:
    ...
    def test_evaluate_shard_rejects_invalid_params(self):
        opt = _make_opt()
        with pytest.raises(TypeError):
            opt.evaluate_shard("not-a-module", 0, _trivial_loss_fn, None, jnp.arange(4, dtype=jnp.int32))  # type: ignore[arg-type]
    ...
    def test_evaluate_shard_matches_full_step(self):
        ...
        for a, b in zip(
            ...
            jax.tree_util.tree_leaves(params_pure_dict(p2)),
        ):
