...
#
# Tuned for INT8 Tensor Core occupancy:
#   - fused D→6D mixer in-proj + wide body (--dim)
#   - memory-light WY (no [C,C,hd] ratios) → larger --batch / --candidate-chunk
#   - moderate --ce-parallel (float CE tiles) so VRAM stays on int8 GEMMs
...
$PYTHON examples/train_int_rnn_minipile.py \
  ...
  --population "${POP:-32}" \
  --rank "${RANK:-4}" \
  --candidate-chunk "${CAND_CHUNK:-16}" \
  --ce-parallel "${CE_PARALLEL:-4}" \
  --delta-impl chunkwise \
  --delta-chunk "${DELTA_CHUNK:-64}" \
  --gdn-feat-tile "${GDN_FEAT_TILE:-32}" \
  --logit-chunk "${LOGIT_CHUNK:-16384}" \
  --prefetch 8 \
  --log-every 10 \
  --gen-every 0 \
  "$@"
